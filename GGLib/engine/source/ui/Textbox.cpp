...
void Textbox::render(Canvas* canvas)
{
	...
	// draw text
	// TODO: BUG: When viewportX > 0, and you delete, then the string texture gets stretched
	int wMinusPadding = w - 2 * PADDING;
	int cursorX;
	canvas->getTextDimensions(text.substr(0, cursorPos), &cursorX, nullptr);
	if (cursorX > viewportX + wMinusPadding)
	{
		viewportX += cursorX - viewportX - wMinusPadding;
	}
	...
	if (textureWidth > wMinusPadding) textureWidth = wMinusPadding;

	SDL_Rect viewport = { viewportX, 0, textureWidth, textureHeight };
