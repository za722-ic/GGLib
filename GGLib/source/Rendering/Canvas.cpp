...
void GG::Canvas::init(SDL_Renderer* newRenderer)
{
	renderer = newRenderer;
}

void GG::Canvas::setAlignment(GG::Canvas::Alignment newAlignment)
{
...
void GG::Canvas::setOriginToCenter()
{
	setOrigin(getWidth() / 2, getHeight() / 2);
}

void GG::Canvas::resetOrigin()
{
	setOrigin(0, 0);
}

void GG::Canvas::setFont(TTF_Font* newFont)
{
	font = newFont;
}

void GG::Canvas::setColor(Uint8 r, Uint8 g, Uint8 b)
{
	setColor(r, g, b, 255);
}
void GG::Canvas::setColor(Uint8 r, Uint8 g, Uint8 b, Uint8 a)
{
	SDL_Color c{ r, g, b, a };
	color = c;
}

void GG::Canvas::setColor(Uint8 c)
{
	setColor(c, c, c, 255);
}

void GG::Canvas::setColor(Uint8 c, Uint8 a)
{
	setColor(c, c, c, a);
}

void GG::Canvas::setColor(SDL_Color c)
{
	color = c;
}
...
int GG::Canvas::getWidth()
{
	int windowWidth;
	SDL_GetRenderOutputSize(renderer, &windowWidth, NULL);

	return windowWidth;
}

int GG::Canvas::getHeight()
{
	int windowHeight;
	SDL_GetRenderOutputSize(renderer, NULL, &windowHeight);

	return windowHeight;
}

SDL_Color GG::Canvas::getColor()
{
	return color;
}

SDL_Renderer* GG::Canvas::getSDLRenderer()
{
	return renderer;
}
...
void GG::Canvas::clear()
{
	setSDLColorToCanvasColor();
	SDL_RenderClear(renderer);
}

void GG::Canvas::present()
{
	SDL_RenderPresent(renderer);
}

void GG::Canvas::drawRect(int x, int y, int w, int h)
{
	setSDLColorToCanvasColor();
...
void GG::Canvas::drawRect(const SDL_Rect& rect)
{
	drawRect(rect.x, rect.y, rect.w, rect.h);
}

void GG::Canvas::drawRect(int x, int y, int w, int h, int thickness)
{
	setSDLColorToCanvasColor();
	applyCanvasOrigin(x, y);
	applyCanvasAlignment(x, y, w, h);
...
void GG::Canvas::drawRect(const SDL_Rect& rect, int thickness)
{
	drawRect(rect.x, rect.y, rect.w, rect.h, thickness);
}

void GG::Canvas::fillRect(int x, int y, int w, int h)
{
	setSDLColorToCanvasColor();
	applyCanvasOrigin(x, y);
	applyCanvasAlignment(x, y, w, h);
...
void GG::Canvas::fillRect(const SDL_Rect& rect)
{
...
void GG::Canvas::drawLine(int x1, int y1, int x2, int y2)
{
	setSDLColorToCanvasColor();
...
GG::Vec2D GG::Canvas::deCasteljau_Cubic(const GG::Vec2D& p0, const GG::Vec2D& p1, const GG::Vec2D& p2, const GG::Vec2D& p3, const float t)
{
	GG::Vec2D B(0);
...
GG::Vec2D GG::Canvas::deCasteljau_Cubic_Derivative(const GG::Vec2D& p0, const GG::Vec2D& p1, const GG::Vec2D& p2, const GG::Vec2D& p3, const float t)
{
	GG::Vec2D dB(0);
...
void GG::Canvas::drawCubicBezier(GG::Vec2D p0, GG::Vec2D p1, GG::Vec2D p2, GG::Vec2D p3, int thickness, int numSegments)
{
...
void GG::Canvas::drawString(std::string text, int x, int y)
{
	int w, h;
...
void GG::Canvas::drawStringToWidth(std::string text, int x, int y, int desiredWidth)
{
...
void GG::Canvas::drawStringToHeight(std::string text, int x, int y, int desiredHeight)
{
...
void GG::Canvas::drawStringToDimensions(std::string text, int x, int y, int w, int h)
{
...
SDL_Texture* GG::Canvas::drawStringToTexture(std::string text)
{
...
void GG::Canvas::getTextDimensions(std::string text, int* w, int* h) // FYI: upon experimentation, passing w or h as NULL does NOT cause an error!
{
...
void GG::Canvas::drawImage(SDL_Texture* image, int x, int y, int w, int h)
{
	applyCanvasOrigin(x, y);
	applyCanvasAlignment(x, y, w, h);
	SDL_FRect rect{ x, y, w, h };
...
void GG::Canvas::drawImage(SDL_Texture* image, int x, int y)
{
	float w, h;
...
void GG::Canvas::drawImageRegion(SDL_Texture* image, const SDL_FRect& region, int x, int y)
{
	drawImageRegion(image, region, x, y, region.w, region.h);
}

void GG::Canvas::drawImageRegion(SDL_Texture* image, const SDL_FRect& region, int x, int y, int w, int h)
{
	applyCanvasOrigin(x, y);
	applyCanvasAlignment(x, y, w, h);
	SDL_FRect rect{ x, y, w, h };
...
void GG::Canvas::drawImage_StrechToFillCanvas(SDL_Texture* image)
{
...
std::vector<SDL_Vertex> GG::Canvas::getRoundedRectVertices(int x, int y, int w, int h, int r, int trianglesPerCorner, SDL_Color color)
{
	std::vector<SDL_Vertex> verts;


	// centers of the quarter circles
	GG::Vec2D cTopRight(x + w - r, y + r);
	GG::Vec2D cTopLeft(x + r, y + r);
	GG::Vec2D cBottomLeft(x + r, y + h - r);
	GG::Vec2D cBottomRight(x + w - r, y + h - r);

	auto appendRoundedCornerVerts = [&](GG::Vec2D center, float startingAngle)
		{
			for (int i = 0; i <= trianglesPerCorner; i++) // using <= because need n + 1 points on circle perimeter for n triangles
			{
				...
				GG::Vec2D vert(cosf(theta), sinf(theta));
...
std::vector<SDL_Vertex> GG::Canvas::interleaveVertices(const std::vector<SDL_Vertex>& v1, const std::vector<SDL_Vertex>& v2)
{
...
std::vector<int> GG::Canvas::getIndices_TriangleFan(int numVertices)
{
...
std::vector<int> GG::Canvas::getIndices_TriangleStrip(int numVertices)
{
	std::vector<int> indices;
...
std::vector<int> GG::Canvas::getIndices_TriangleStrip_Loop(int numVertices)
{
...
std::vector<SDL_Vertex> GG::Canvas::generateArcVertices(int x, int y, int w, int h, float startAngle, float endAngle, int numTriangles, bool isChord)
{
	/*
	...
	*/
	...
	for (int i = 0; i <= numTriangles; i++)
	{
		...
		GG::Vec2D v(rHor * std::cos(angle), rVer * std::sin(angle));
...
std::vector<SDL_Vertex> GG::Canvas::generateArcNormals(int x, int y, int w, int h, float startAngle, float endAngle, int numTriangles, int thickness)
{
	...
	*/
	...
	for (int i = 0; i <= numTriangles; i++)
	{
		...
		GG::Vec2D n(rVer * std::cos(angle), -rHor * sin(angle));
		n.normalize();
		n *= thickness;
...
void GG::Canvas::drawArc(int x, int y, int w, int h, float startAngle, float endAngle, int numTriangles, int thickness)
{
...
void GG::Canvas::fillArc(int x, int y, int w, int h, float startAngle, float endAngle, int numTriangles, bool isChord)
{
...
void GG::Canvas::renderRegularPolygon(int x, int y, unsigned int numVertices, float rotAngle, float scale)
{
	...
	for (int i = 0; i < numVertices; i++)
	{
		...
		GG::Vec2D v1(x + scale * cosf(angle), y + scale * sinf(angle));

		angle = rotAngle + INTERVAL * (i + 1);

		GG::Vec2D v2(x + scale * cosf(angle), y + scale * sinf(angle));
...
void GG::Canvas::setSDLColorToCanvasColor()
{
	SDL_SetRenderDrawColor(renderer, color.r, color.g, color.b, color.a);
}

void GG::Canvas::applyCanvasOrigin(int& x, int& y)
{
	x += originX;
	y += originY;
}

void GG::Canvas::applyCanvasAlignment(int& x, int& y, int w, int h)
{
...
GG::Canvas::HorizontalAlignment GG::Canvas::getHorizontaAlignment(Alignment alignment)
{
	switch (alignment)
	{
	case GG::Canvas::Alignment::TOP_LEFT:
	case GG::Canvas::Alignment::CENTER_LEFT:
	case GG::Canvas::Alignment::BOTTOM_LEFT:
		return HorizontalAlignment::LEFT;
	case GG::Canvas::Alignment::TOP_CENTER:
	case GG::Canvas::Alignment::CENTER_CENTER:
	case GG::Canvas::Alignment::BOTTOM_CENTER:
		return HorizontalAlignment::CENTER;
	case GG::Canvas::Alignment::TOP_RIGHT:
	case GG::Canvas::Alignment::CENTER_RIGHT:
	case GG::Canvas::Alignment::BOTTOM_RIGHT:
		return HorizontalAlignment::RIGHT;
	}

	throw std::runtime_error("Unknown Alignment type");
}
GG::Canvas::VerticalAlignment GG::Canvas::getVerticalAlignment(Alignment alignment)
{
	switch (alignment)
	{
	case GG::Canvas::Alignment::TOP_LEFT:
...
void GG::Canvas::pushClipRect(const SDL_Rect& clipRect)
{
	clipRectStack.push_back(clipRect);

	calculateActiveClipRect();
}

void GG::Canvas::popClipRect()
{
...
void GG::Canvas::calculateActiveClipRect()
{
	// set active clip rect to be be whole screen (i.e., essentially no clipping)
	activeClipRect = { 0,0, getWidth(), getHeight()};

	// we want active clip rect to be the region that intersects all clip rects on the stack
	for (const auto& clipRect : clipRectStack)
	{
		bool doesIntersectionExit = SDL_GetRectIntersection(&clipRect, &activeClipRect, &activeClipRect);

		// the result of intersecting all clip rects on the stack is that there is no region of intersection
		// in this case, we don't want to render anything
		if (!doesIntersectionExit)
		{
			activeClipRect = clipRectForNoRendering;
			break;
		}
	}

	// tell SDL to use the new activeClipRect
